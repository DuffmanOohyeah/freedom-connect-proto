...
import NoDataCancellation from '../actionButtons/NoDataCancellation';
import { ArrowsPointingOutIcon } from '@heroicons/react/24/outline';
import { useSession } from 'next-auth/react';
import { getCurrentBusinessUnit } from '@/app/utils/getCurrentBusinessUnit';

const ReportingDeviceNoData = (): JSX.Element => {
	const { state } = useContext(BusinessUnitCtx)!;
	const { data: session } = useSession();
	const [reports, setReports] = useState<DeviceNoDataReport[]>([]);
	...
	const columnHelper = createColumnHelper<DeviceNoDataReport>();
	const columns = [
	...
	const fetchDeviceNoData = async (uid: number): Promise<void> => {
		setIsLoading(true);
		setIsError(false);

		if (uid > 0) {
			const response = await fetch(
				`${BaseUrl}/portal/report/device/nodata?uid=${uid}`,
				{
					method: 'GET',
			...
			const resPolicies =
				(await response.json()) as GetDeviceNoDataReport;
			setReports(resPolicies.report || []);
			setFilteredReports(resPolicies.report || []);
		}
	...
	useEffect(() => {
		...
		fetchDeviceNoData(currentBU.id || 0);
	}, [session, state]);
...
export default ReportingDeviceNoData;
