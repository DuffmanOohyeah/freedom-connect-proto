...
interface ResendUnplugCommsProps {
	policy: GetPolicyRes | null;
}

const ResendUnplugComms = ({ policy }: ResendUnplugCommsProps): JSX.Element => {
	const users: User[] = policy?.users || [];
	const [errorMessage, setErrorMessage] = useState<string>('');
	const [success, setSuccess] = useState<boolean>(false);
	const [selectedUser, setSelectedUser] = useState<User | undefined>();

	const action = useMutation(['resendUnplugComms'], {
		mutationFn: async (): Promise<boolean> => {
			const response = await fetch(`${BaseUrl}/portal/comm/resend`, {
				method: 'POST',
				body: JSON.stringify({
					commType: 'RESEND_UNPLUG_COMMS',
		...
		<>
			<div>
				...
				<p className='text-xs text-center mt-12'>
					Your are about to resend the unplug comms email to{' '}
...
export default ResendUnplugComms;
