...
const UnplugCancellation = ({
	...
	const action = useMutation(['deviceUnplugCancellation'], {
		mutationFn: async (): Promise<boolean> => {
			const response = await fetch(
				`${BaseUrl}/portal/device/cancel-for-unplug`,
				{
					method: 'POST',
					body: JSON.stringify({
						opid: opid,
		...
		<div className='flex flex-col justify-between h-full'>
			<div>
				...
				<p className='text-xs text-center mt-12'>
					You are about to raise a cancellation for device unplug.
					(OPID: {opid} Barcode:{' '}
...
export default UnplugCancellation;
